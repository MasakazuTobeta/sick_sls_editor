...
2. Flask を起動します。
   ...
   python main.py
   ```
3. 同じホストでは http://localhost:5000/ または http://127.0.0.1:5000/、LAN 上の別ホストでは `http://<起動ホストのIPアドレス>:5000/` をブラウザで開いて UI を確認します。`python main.py` は全 IPv4 インターフェースで待ち受けます。別ホストから接続できない場合は、起動ホストのファイアウォールで TCP 5000 番ポートが許可されているか確認してください。

`python main.py` はデバッグ機能付きの開発サーバーです。信頼できる開発用 LAN 内だけで使用し、インターネットや信頼できないネットワークには公開しないでください。
